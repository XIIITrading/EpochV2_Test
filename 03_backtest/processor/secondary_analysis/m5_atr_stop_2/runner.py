...
M5 ATR Stop Processor v2 - CLI Runner
XIII Trading LLC
================================================================================

Command-line interface for running the M5 ATR Stop calculation.
...
def run_schema():
    """Execute the schema SQL to create the m5_atr_stop_2 table."""
    import psycopg2

    schema_file = SCHEMA_DIR / "m5_atr_stop_2.sql"
    ...
    print("=" * 60)
    print("M5 ATR Stop v2 - Schema Creation")
    print("=" * 60)
...
def show_info():
    """Display information about the M5 ATR Stop processor."""
    print()
    print("=" * 60)
    print("M5 ATR STOP PROCESSOR v2")
    print("=" * 60)
    print()
    print("PURPOSE:")
    print("  Evaluate trades using M5 ATR(14) as the stop distance (1R)")
    print("  and track R-multiple targets (1R-5R) to determine win/loss outcomes.")
    print("  Wider stop than M1 ATR — useful for cross-timeframe comparison.")
    print()
    print("STOP CONFIGURATION:")
    print("  ATR Source:     m1_indicator_bars_2.atr_m5 (pre-computed)")
    print("  ATR Period:     14")
    ...
    print("  -1 for LOSS. Highest R-level (1-5) hit before stop_time for WIN.")
    print("  AVG(max_r) = net expectancy in R.")
    print()
    ...
    print("  - m1_indicator_bars_2 (pre-computed atr_m5 at entry candle)")
    print()
    print("TARGET TABLE:")
    print("  - m5_atr_stop_2 (1 row per trade)")
    print()
    print("=" * 60)


def run_calculation(args):
    """Run the M5 ATR Stop calculation."""
    print()
    print("=" * 60)
    print("EPOCH TRADING SYSTEM")
    print("M5 ATR Stop Calculator v1.0.0")
    print("=" * 60)
    print()
    ...
    # Create calculator
    calculator = M5AtrStopCalculator(verbose=args.verbose)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Calculate M5 ATR Stop outcomes using M5 ATR(14) stop and R-multiple targets (1R-5R)',
...
Output:
  Results are written to the m5_atr_stop_2 table in Supabase.
  Each trade gets 1 row with R-level hit tracking and outcome.
    ...
    parser.add_argument(
        '--schema',
        action='store_true',
        help='Run schema creation only (create m5_atr_stop_2 table)'
    )

    parser.add_argument(
        '--info',
        action='store_true',
        help='Display information about the M5 ATR Stop processor'
    )
