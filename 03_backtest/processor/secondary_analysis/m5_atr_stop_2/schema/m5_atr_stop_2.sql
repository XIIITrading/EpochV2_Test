-- ============================================================================
-- Epoch Trading System - Table: m5_atr_stop_2
-- M5 ATR Stop Analysis - R-Multiple Target Evaluation
-- XIII Trading LLC
--
-- PURPOSE:
--   Evaluates each trade using M5 ATR (14-period, 1x) as the stop/risk unit.
...
--
--   The M5 ATR provides a wider stop than M1 ATR, useful for comparing trade
--   outcomes across volatility timeframes.
--
-- WIN/LOSS LOGIC:
...
--   - M5 ATR: m1_indicator_bars_2 table (pre-computed atr_m5 at entry candle)
--
-- Version: 1.0.0
-- ============================================================================
...
-- COMMENTS
-- ============================================================================
COMMENT ON TABLE m5_atr_stop_2 IS 'M5 ATR stop analysis: evaluates trades using M5 ATR(14) stop with R-multiple targets (1R-5R). M1 bar simulation fidelity. 1 row per trade.';
COMMENT ON COLUMN m5_atr_stop_2.m5_atr_value IS 'Raw 14-period ATR on M5 bars at entry candle time (from m1_indicator_bars_2.atr_m5)';
COMMENT ON COLUMN m5_atr_stop_2.m1_entry_candle_adj IS 'Entry time truncated to containing M1 candle (seconds zeroed)';
COMMENT ON COLUMN m5_atr_stop_2.stop_price IS 'M5 ATR stop price: entry -/+ m5_atr_value depending on direction';
COMMENT ON COLUMN m5_atr_stop_2.stop_distance IS 'Dollar distance from entry to stop = 1R unit = m5_atr_value';
COMMENT ON COLUMN m5_atr_stop_2.r1_hit IS 'Did price reach 1R target before stop?';
COMMENT ON COLUMN m5_atr_stop_2.r1_time IS 'Time when R1 was first reached';
COMMENT ON COLUMN m5_atr_stop_2.r1_bars_from_entry IS 'M1 bars from entry to R1 hit';
COMMENT ON COLUMN m5_atr_stop_2.max_r IS 'R-multiple result: -1 = LOSS (stopped out), 1-5 = highest R-level hit before stop. AVG(max_r) = net expectancy in R.';
COMMENT ON COLUMN m5_atr_stop_2.result IS 'WIN if R1 hit before stop, LOSS otherwise';
COMMENT ON COLUMN m5_atr_stop_2.stop_time IS 'Time when stop was first triggered (M1 close beyond stop level)';
...
-- 5. Compare M1 vs M5 ATR stop outcomes for the same trades
SELECT
    m1.trade_id, m1.ticker, m1.date, m1.direction, m1.model,
    m1.m1_atr_value as m1_atr, m5.m5_atr_value as m5_atr,
