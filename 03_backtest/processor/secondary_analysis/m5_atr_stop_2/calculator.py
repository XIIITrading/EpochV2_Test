...
M5 ATR Stop Calculator
XIII Trading LLC
================================================================================

Evaluates trades using M5 ATR(14) as the stop distance (1R) and tracks
R-multiple targets (1R through 5R) to determine win/loss outcomes.

The M5 ATR provides a wider stop than M1 ATR. The bar-by-bar simulation
still walks M1 bars from m1_bars_2 for maximum detection fidelity.

LOGIC:
    1. Read entry from trades_2, adjust entry_time to M1 candle (truncate seconds)
    2. Fetch pre-computed atr_m5 from m1_indicator_bars_2 at adjusted entry candle
    3. Set stop = entry -/+ atr_m5 (no multiplier) => this distance = 1R
...
DATA SOURCES (v2 tables):
    - trades_2 (trade metadata)
    - m1_bars_2 (M1 candle data for bar-by-bar simulation)
    - m1_indicator_bars_2 (pre-computed atr_m5 at entry candle)
...
from config import (
    DB_CONFIG, EOD_CUTOFF, R_LEVELS, SOURCE_TABLES, TARGET_TABLE
)
...
@dataclass
class M5AtrStopResult:
    """Result of M5 ATR Stop calculation for a single trade."""
...
class M5AtrStopCalculator:
    """
    Calculates M5 ATR Stop outcomes using pre-computed M5 ATR and R-multiple targets.

    This class:
    1. Queries trades_2 for trades not yet in m5_atr_stop_2
    2. Adjusts entry time to containing M1 candle
    3. Fetches pre-computed atr_m5 from m1_indicator_bars_2
    4. Calculates stop price and R-level targets (wider than M1 ATR)
    5. Walks M1 bars to detect R-level hits and stop triggers
    6. Writes results to m5_atr_stop_2 table
    """
    ...
    def get_trades_needing_calculation(self, conn, limit: int = None) -> List[Dict[str, Any]]:
        """
        Query trades_2 for trades not yet processed in m5_atr_stop_2.
    ...
    def get_m5_atr_at_entry(
        ...
        """
        Fetch pre-computed atr_m5 from m1_indicator_bars_2 at the adjusted entry candle.
        ...
        Returns:
            atr_m5 value or None if not available
        """
        query = f"""
            SELECT atr_m5
            FROM {SOURCE_TABLES['m1_indicator_bars']}
    ...
    def get_m1_bars(
        ...
        """Fetch M1 bars for a ticker/date from m1_bars_2."""
        query = f"""
    ...
    def insert_results(self, conn, results: List[M5AtrStopResult]) -> int:
        """Insert calculation results into m5_atr_stop_2 table."""
        if not results:
            return 0
        ...
        values = [
            (
                r.trade_id, r.date, r.ticker, r.direction, r.model, r.zone_type,
    ...
    def calculate_single_trade(
        ...
        m5_atr_value: float
    ) -> Optional[M5AtrStopResult]:
        """
        Calculate M5 ATR Stop outcome for a single trade.
        ...
        # Stop distance = M5 ATR (1x, no multiplier)
        stop_distance = m5_atr_value
        ...
        result = M5AtrStopResult(
            ...
            m5_atr_value=round(m5_atr_value, 6),
    ...
    def run_batch_calculation(
        ...
        """
        Main entry point. Process all trades needing M5 ATR Stop calculation.
        ...
        print("=" * 60)
        print("M5 ATR Stop Calculator")
        print("=" * 60)
        print(f"Stop Type: M5 ATR(14) \u00d7 1.0")
        ...
        try:
            ...
            for idx, trade in enumerate(trades):
                ...
                # Step 2: Fetch M5 ATR at adjusted entry candle
                m5_atr = self.get_m5_atr_at_entry(conn, ticker, trade_date, m1_candle)
                if m5_atr is None or m5_atr <= 0:
                    self._log(
                        f"Skipping {trade_id}: no M5 ATR at {ticker} {trade_date} {m1_candle}",
                        'warning'
                    )
                    self.stats['trades_skipped'] += 1
                    continue
                ...
                # Step 4: Calculate
                try:
                    result = self.calculate_single_trade(trade, m1_bars, m5_atr)
...
if __name__ == "__main__":
    print("M5 ATR Stop Calculator - Test Mode")
    print("=" * 60)

    calculator = M5AtrStopCalculator(verbose=True)
